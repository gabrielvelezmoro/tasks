...
export class TokenService {
  ...
  public async generateResetPassToken(user: User): Promise<Token> {
    ...
    const resetPassToken = await this.generateJwtToken(
      user,
      TOKEN_CONFIG.EXP.resetPassTokenExp,
      TOKEN_TYPE.RESET_PASSWORD,
    );
    const resetPassTokenEntity = this.tokenRepository.create({
  ...
  public async verifyToken(
    ...
    const tokenRecord = await this.tokenRepository.findOne({
      where: { token: token, type: tokenType },
    });
